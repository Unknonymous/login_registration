...
class MySQLConnection(object):
    ...
    def query_db(self, query, data=None):
        ...
        if query[0:6].lower() == 'select':                      #if the query was a select
            list_result = [dict(r) for r in result]             #convert it to a list of dictionaries
            return list_result                                  #return the result as a list of dictionaries
        
        elif query[0:6].lower() == 'insert':                    #if the query was an insert
            self.db.session.commit()                            #commit changes
            return result.lastrowid                             #return the id of the row that was inserted

        else:                                                   #if the query was an update or delete...
            self.db.session.commit()                            #return nothing and commit changes
